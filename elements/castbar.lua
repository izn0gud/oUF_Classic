...
local function CastStart(self, event, unit)
	...
	local name, _, texture, startTime, endTime, isTradeSkill, castID, notInterruptible, spellID = CastingInfo()
	event = 'UNIT_SPELLCAST_START'
	if(not name) then
		name, _, texture, startTime, endTime, isTradeSkill, notInterruptible, spellID = ChannelInfo()
...
local function CastUpdate(self, event, unit, castID, spellID)
	...
	if(event == 'UNIT_SPELLCAST_DELAYED') then
		name, _, _, startTime, endTime = CastingInfo()
	else
